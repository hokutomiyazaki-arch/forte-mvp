...
※【STOP 1 で確定】「定着」= 管理ダッシュボードの集計定義（`src/app/admin/dashboard/page.tsx:787-790, 830-833`）：
`vote_type='proof'` の全票（`fetchAllProofVotes`、`page.tsx:368-386`。status フィルタなし・実データは全件 confirmed）を JST 日付に丸め、**投票日が2日以上あるプロ = 定着**（1日のみ=お試し／0=登録のみ）。
実データ再計算：**定着 111人**（指示書の「110人」と整合）／お試し 49人。
**定着111人のうちプルーフ15件以上（vote_summary 定義・重み無し）: 38人**。定着プロのプルーフ数 中位値=10件（p25=5 / p75=20）。
（15件以上のプロ38人は全員が定着に含まれる＝全体160人中の38人と同一集合）
