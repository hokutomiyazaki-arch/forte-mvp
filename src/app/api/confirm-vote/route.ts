...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Step 8: Clerkユーザー作成 + clients upsert
    let clerkUserId: string | null = null
    try {
      const clerk = await clerkClient()
      if (!hasAccount) {
        // 新規Clerkユーザーを作成（パスワードなし）
        const newUser = await clerk.users.createUser({
          emailAddress: [vote.voter_email],
        })
        clerkUserId = newUser.id
        console.log('[confirm-vote] Step 8 OK - Clerk user created:', clerkUserId)
      } else {
        // 既存ユーザーのClerk IDを取得
      ...
      console.error('[confirm-vote] Step 8 - Clerk user create/fetch error:', e.message)
    }

    // clients テーブルに upsert
    if (clerkUserId) {
      try {
        const nickname = vote.voter_email.split('@')[0]
        await supabaseAdmin
          .from('clients')
          .upsert(
            {
        ...
        console.log('[confirm-vote] Step 8b OK - clients upserted for:', clerkUserId)
      } catch (e: any) {
        console.error('[confirm-vote] Step 8b - clients upsert error:', e.message)
      }
    }

    // has_account=false の場合のみパスワード設定メールを送信
    const setupResendKey = process.env.RESEND_API_KEY
    if (!hasAccount && clerkUserId && setupResendKey) {
      try {
        const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://forte-mvp.vercel.app'
        await fetch('https://api.resend.com/emails', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${setupResendKey}`,
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            from: 'REAL PROOF <info@proof-app.jp>',
            to: vote.voter_email,
            subject: '【REALPROOF】リワードをいつでも確認できます ✨',
            html: `
              <div style="font-family:sans-serif;max-width:480px;margin:0 auto;">
                ...
                <div style="padding:24px;background:#fff;border:1px solid #eee;">
                  <h2 style="color:#1A1A2E;font-size:18px;">投票が完了しました！</h2>
                  <p style="color:#333;">リワードはマイページでいつでも確認できます。</p>
                  <p style="color:#333;">以下のリンクからパスワードを設定してください：</p>
                  <div style="text-align:center;margin:24px 0;">
                    <a href="${appUrl}/sign-up?email=${encodeURIComponent(vote.voter_email)}"
                       style="display:inline-block;background:#C4A35A;color:#fff;padding:14px 32px;border-radius:8px;text-decoration:none;font-size:14px;">
                      パスワードを設定する →
                    </a>
                  </div>
                </div>
                <div style="padding:16px;text-align:center;background:#f9f9f9;border-radius:0 0 12px 12px;">
                  <p style="color:#999;font-size:11px;margin:0;">このメールに心当たりがない場合は無視してください。</p>
                </div>
              </div>
            `,
          }),
        })
        console.log('[confirm-vote] Step 8c OK - password setup email sent to:', vote.voter_email)
      } catch (e: any) {
        console.error('[confirm-vote] Step 8c - password setup email error:', e.message)
      }
    }
