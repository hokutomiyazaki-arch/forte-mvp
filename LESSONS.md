...
### J-x. 「同じものを数えているつもり」の集計が4箇所でバラバラだった（2026-08-06・admin 日別プルーフ獲得者）

**症状**: admin の「日別プルーフ獲得者」に出ないプロがいる。団体ダッシュボードにはそのプロのコメントが出ているし、
公開カードの Voices にも 8/5・8/3 の記録がある。各行の投票数がどれも「1」で、CEO の見立ては
「新しい人からで計測してる？プルーフ獲得ではなく」。

**真因**: `vote_type` を `'proof'` だけで絞っていた。投票画面で「2回目以降」を選ぶと
`determineVoteType()` は **`'continuation'`** を返す（`vote/[id]/page.tsx`）。つまり
**リピーターの記録が全部落ちていた**。落ちた結果、表に残るのは初回票のある日だけになり、
どの行も「1」に見えていた。CEO の見立ては本質的に正しく、「初回の票しか数えていない」状態だった。

**背景（本当の問題）**: 同じ「プルーフ」を数える式がコードベースに4つあった。

| 場所 | status | vote_type |
|---|---|---|
| admin 日別プルーフ獲得者 | confirmed | `= 'proof'` ← 最も狭い |
| `/api/vote-count`（クライアントの「あなたの記録」） | confirmed | `IN ('proof','continuation')` |
| 公開カード Voices | confirmed | フィルタなし |
| 団体ダッシュボードの最新コメント | **フィルタなし** | フィルタなし |

「団体には出るのに admin に出ない」のはこの差そのもの。**どれか1つが壊れていたのではなく、
定義が4通りあった**のが問題。

**教訓**:
- 新しい `vote_type` を足したら、**既存の `.eq('vote_type', ...)` を全部grepして棚卸しする**。
  値を1つ足すことは、`eq` で絞っている既存の集計すべてを黙って狭めることと同じ。
- 「プルーフ数」のような**プロダクトの中心的な数の定義は1箇所に置く**。式をコピーした時点でズレる。
- 「全部の行が同じ値（ここでは1）」は集計バグのサイン。データがそうなのではなく、
  **絞り込みが効きすぎている**ことを先に疑う。

**同時に見つかった別バグ**: `created_at.split('T')[0]` は timestamptz が UTC で返るため **UTC の日付**。
日本時間 00:00〜09:00 の出来事が前日に計上されていた。日別集計3箇所（プルーフ獲得者・トレンド・新規登録）を
...
## 2026-08-07 深夜（予約・相談まわりの本番事故5件）

詳細は `docs/referral/RP_REFERRAL_IMPL_SPEC.md` §17-16〜17-25、
経緯は `docs/handoff/2026-08-07-night.md`。

### L-1. 正規化したメールアドレスを宛先に使わない（§17-20）

`consultations.client_email` に `normalizeEmail()` の結果を保存していた。
この関数は**照合キー**を作るもので、全ドメインで `+タグ` 以降を落とし、Gmailはドットも落とす。
結果、`foo+test@example.com` 宛のつもりのメールが `foo@example.com`（**別のメールボックス**）へ
飛んでいた。Gmailのドットだけなら偶然届くので「ときどき届く」に見える。

> **正規化したアドレスは「同じ人か」を判定するキーであって、宛先ではない。**
> 送信・表示に使う値は、必ず本人が入力したものを保存する。

同じ根で逆向きにも壊れていた: バウンス通知は生アドレスで来るのに、正規化してから
生アドレス保存の `referral_bookings` を `.in()` で探していたため、`+タグ` では永久に一致しなかった。
**照合するときは、両辺がどちらの形で保存されているかを必ず確かめる。**

### L-2. 制限は「捨てる」のではなく「受け止める」（§17-21）

相談の連投防止（5分）が、既存スレッドへリダイレクトするだけで
**入力された本文を1件も保存していなかった**。送った側は「送信した」のに、
飛ばされた先に自分の文章が無い＝「表示されない」に見える。

制限の狙いは「**スレッドを増やさない**」ことであって「本文を捨てる」ことではなかった。
**入力を受け取ったあとに弾く分岐を書くときは、その入力がどこへ行くのかを必ず決める。**

### L-3. 例外を持ち回すときは、その理由がその場所にも当てはまるか確かめる（§17-22）

「予約金が絡むときは逃げ道を残す」は、削除が409になる**操作メニュー**に必要な例外だった。
これを隣の「メッセージを送る」の表示条件にコピーしたため、メールが死んでいるのに
チャットボタンが出ていた（チャットの通知はメールなので、支払い状況に関係なく届かない）。

### L-4. 見た目を揃えるだけでなく、置き場所も合わせる（§17-23）

告知バナーの CSS を `NoticeBannerShell` に共通化して満足し、**マウント位置**を確認しなかった。
admin配信は `(main)/layout.tsx` の Navbar 直下・`<main>` の外（全幅）。
こちらはページ本文の中に置いていたため、幅も余白も別物に見えた。

### L-5. 告知に、ユーザーの画面に見えていない用語を書かない（§17-24）

仕様書を見て告知文を書いたため、先行公開中で**アローリスト外のプロには存在しない**
予約金・紹介リストの説明が混ざった。「予約金はかかりません」は、予約金を知らない人には
意味が通らないどころか「予約金って何？お金がかかるの？」という不安だけを残す。

> 文面を書く前に、**その機能がいまそのユーザーの画面に出ているか**を確認する。

### L-6. 「外部からの通知は毎回来る」を前提にしない（§17-25）

Resend は一度ハードバウンスしたアドレスを**抑制リスト**に入れ、次からは送信もせず
`email.bounced` も飛ばさない。そのため「初めての打ち間違い」は検知でき、
「前にも使った打ち間違い」は**永久に検知できない**状態になっていた。

実運用でも「アドレスを間違えた人が直さずにもう一度予約する」で普通に起きる。
しかも2回目以降のほうが放置されやすい（画面に何も出ないため誰も気づかない）。

> 外部サービスの通知に依存する検知は、**こちらが既に知っている事実**で補強する。

### L-7. Clerkのセッショントークンは約60秒（§17-18）

スマホでタブを放置・画面消灯・別アプリへ切替をすると、クライアント側の更新タイマーが止まる。
復帰後の**最初の1回だけ** 401 になり、その後 Clerk が更新するので2回目以降は通る。
「久しぶりに操作した最初の1回だけ落ちる」という形で出る。コードにもデータにも問題はない。

対処: 401 なら `getToken({ skipCache: true })` で取り直し、**黙って1回だけ再送**する
（`src/lib/fetch-with-session-retry.ts`）。英語の "Unauthorized" を画面に出さない。

### L-8. 推測で原因を断定しない。先にデータを見る

「相談メールが届かない」を `normalizeEmail` のバグと結び付けたが、実データを見たら
`+` もドットも無く、**単に打ち間違いアドレス**だった（CEOがバウンステスト用に入れていたもの）。
バグ自体は本物だったが、その症状の原因ではなかった。
**症状 → 仮説 → データで確認 → 断定**の順を飛ばさない。
なお、このとき出した診断SQLも JOIN 条件が効いておらず総当たりになっていた。
**自分が書いたクエリの結果を、検証せずに証拠として使わない。**
