...
- **【恒久ルール・2026-08-06 CEO決定】外出中（スマホから起動した web セッション）は上記の例外。**
  ...
  - **main への取り込みは PR 方式（CEO決定・2026-08-06）。** web セッションは main に直接 push できずブランチ指定に
    なるため、作業が一区切りしたら **CC が PR を作る**。
    - **【2026-08-06・CEO決定・運用変更】マージまで CC がやる。**「だからマージpushはあなたがやるの！！」。
      PR を作って CEO のマージ待ちにしない。**PR 作成 → 自分でマージ**までを一連で完了させ、
      結果（本番に出た／Vercel が反映中）を報告する。CEO にマージを押させない。
      旧ルール（ほくとがスマホの GitHub アプリで Merge を押す）は**この指示で撤回**。
      ※ PR を挟むこと自体は維持（差分の記録が残る）。CC が main に直接 push することはしない。
    - マージ後に続きの作業をするときは、ブランチを最新の main から取り直す
      （`git fetch origin main && git checkout -B <branch> origin/main`）。マージ済みの PR は再利用しない。
...
- **【恒久ルール・2026-08-06 CEO決定】新機能を出したら毎回まとめて告知する。**
  「新機能が追加され続けている」こと自体が、現時点でユーザーの興味をつなぎとめる一番のポイント。
  作って終わりにしない。CC は一区切りごとに **①アプリ内バナー ②一斉送信（メール/LINE）の文面を
  必ず用意する**（`docs/announcements/YYYY-MM-DD-*.md` に置く）。CEOに催促されてから書かない。
  - 文面は「何ができるようになったか」を先に、操作手順は後。プロが得することだけ書く。
...
- **【恒久ルール・2026-08-06 CEO決定・運用変更】push 前に必ず「依存インストール → 型チェック → ビルド」を通す。**
  ...
  - 事故: web セッションのコンテナは **node_modules が入っていない**。その状態で `npx tsc --noEmit` を
    実行すると新しい TypeScript が落ちてきて、tsconfig の `target: es5` に対する **TS5107 で
    ファイルを1つも検査せず終了**する。CCはそれを「型OK」と誤判定し、**6コミット連続で
    Vercel ビルドを壊したまま push した**（本番が30分近く古いまま止まった）。
  - 以後の手順（web セッション）:
    1. `npm ci`（node_modules が無ければ必ず最初に）
    2. `npx tsc --noEmit` → **exit code が 0 であることを確認**（出力の grep で判断しない）
    3. `npx next build` → 成功を確認
    4. それから commit / push
  - **エラー出力を grep で除外して「OK」と判断しない。** 除外した中に本体があった。
  - PC で作業する時は従来どおりビルドはほくとが手動でよい。

- **【恒久ルール・2026-08-06 CEO決定】新機能を作る前に、同じ仕組みを持つ他業種のサービスを調べる。**
  相談チャットの4点強化（§16-27）は**シティヘブンの事前チャット**から学んだもの。
  「返信期待値」「連投制限」「相談→予約の接続」「通報＋非閲覧の明記」は、
  先行サービスが実際に踏んだ失敗の対策として存在している。
  - 実装前に **①同種の仕組みを持つ他業種のサービスを2〜3挙げる ②そこで起きている失敗・
    苦情の型を挙げる ③今回の設計でそれが起きないかを確認する** を必ず行い、指示書に残す。
  - 自分たちで踏んでから直すのではなく、**先に潰す**。UIの細部より、こちらのほうが効く。

- **【恒久ルール・2026-08-06 CEO決定】指示書より CEO の最新の発言が正。**
  「あなたの持ってる指示書よりも俺が最新。前提が変わったのに古い指示にしがみつかないで。」
  - CEO の言うことが指示書（`RP_REFERRAL_IMPL_SPEC.md`）と矛盾したとき、**指示書を根拠に
    「仕様ではこうです」と返さない**。前提が更新されたと考えるのが既定。
  - ただし黙って上書きもしない。**古い指示にどういう合理性があったかを必ず考え**、
    それが今も生きているなら「この理由で入れていた制約です」と伝えたうえで、最新の判断に従う。
  - 反映したら指示書を更新し、**古い記述は消さずに「◯◯により変更」と履歴を残す**
    （なぜ変わったかが消えると、また同じ議論を繰り返す）。
  - 事故例: 受付トグルについて CEO が「予約受付にする」と決めていたのに、CC が指示書と
    コードを根拠に「これは紹介の受付です」と説明した。前提が変わっていた。

- `npm run build` は上記のとおり **CC が実行する**（旧: ほくと手動）。
